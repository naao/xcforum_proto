...
{
	return xcforum_notify_base( "'.$mydirname.'" , $category , $item_id ) ;
}
' ) ;

if( ! function_exists( 'xcforum_notify_base' ) ) {
...
function xcforum_notify_base( $mydirname , $category , $item_id )
{
	...
	if( $category == 'category' ) {
		...
		$whr_cat = 'cat_id IN ('.implode(',',xcforum_get_categories_can_read( $mydirname )).')' ;
	...
	if( $category == 'forum' ) {
		...
		$whr_forum = 'forum_id IN ('.implode(',',xcforum_get_forums_can_read( $mydirname )).')' ;
