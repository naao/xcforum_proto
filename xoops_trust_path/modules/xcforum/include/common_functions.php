...
function xcforum_get_forums_can_read( $mydirname )
{
	global $xoopsUser ;
...
function xcforum_get_categories_can_read( $mydirname )
{
	global $xoopsUser ;
...
function xcforum_get_submenu( $mydirname )
{
...
function xcforum_restruct_categories( $categories , $parent )
{
	$ret = array() ;
...
function xcforum_common_is_necessary_antispam( $user , $mod_config )
{
...
function &xcforum_common_get_antispam_object( $mod_config )
{
...
function xcforum_common_unhtmlspecialchars( $text )
{
	return strtr( $text , array_flip( get_html_translation_table( HTML_SPECIALCHARS , ENT_QUOTES ) ) + array( '&#039;' => "'" ) ) ;
}


function xcforum_common_simple_request( $params )
{
	$myts =& MyTextSanitizer::getInstance() ;
	$requests = array() ;
	$whrs = array() ;
...
function xcforum_common_utf8_encode_recursive( &$data )
{
	if( is_array( $data ) ) {
		foreach( array_keys( $data ) as $key ) {
			xcforum_common_utf8_encode_recursive( $data[ $key ] ) ;
		}
